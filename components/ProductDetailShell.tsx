          ...
          <div className="grid gap-10 lg:grid-cols-[1.4fr_1fr] xl:grid-cols-[1.5fr_1fr]">
            <div className="space-y-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-5">
                  <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Current deal</p>
                ...
                <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-5">
                  <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Seller reliability</p>
              ...
              <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
                <p className="text-sm uppercase tracking-[0.3em] text-slate-400">Select your variant</p>
                <div className="mt-4 flex flex-wrap gap-3">
                  {variantLabels.map((label, index) => (
                    <button
                      key={label}
                      onClick={() => setActiveVariant(index)}
                      className={`rounded-3xl border px-4 py-3 text-sm transition ${index === activeVariant ? 'border-cyan-400 bg-cyan-500/15 text-cyan-200' : 'border-slate-700 bg-slate-950 text-slate-300 hover:border-slate-500'}`}
                    >
                      {label}
                    </button>
                  ))}
            ...
            <div className="space-y-6 rounded-3xl border border-slate-800 bg-slate-950/90 p-6">
              <div className="rounded-3xl border border-slate-800 bg-slate-900/90 p-6">
                ...
                <p className="mt-4 text-lg text-white">Buy now or wait?</p>
                <div className="mt-6 space-y-3 text-slate-300">
                  <p>Current price is {product.currentPrice < product.mrp ? 'below average' : 'within expected range'} for this category.</p>
                  <p>Price history shows {product.priceHistory.length} trend points over the last year.</p>
                  <p className="flex items-center gap-2 text-cyan-300"><Sparkles className="h-4 w-4" />Recommended: purchase with a matched warranty and accessory bundle.</p>
                </div>
              </div>
              <div className="rounded-3xl border border-slate-800 bg-slate-900/90 p-6">
            ...
            <article className="rounded-3xl border border-slate-800 bg-slate-900/90 p-6">
              ...
              <div className="mt-6 space-y-4">
                {product.priceHistory.slice(-4).map((point) => (
                  <div key={point.timestamp.toISOString()} className="flex items-center justify-between text-sm text-slate-300">
